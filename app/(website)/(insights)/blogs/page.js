    ...
    <>
      <div className="max-w-7xl mx-auto px-4 py-10 mt-24 bg-white">
        <div className="flex flex-col mb-10 justify-between sm:items-center sm:flex-row gap-6">
          <div>
            <h1 className="text-5xl font-normal">Blogs</h1>
          </div>
          <InsightActions />
        </div>
      ...
      {pageCount > 1 && (
        <div className="flex justify-center bg-white">
          <InsightPagination page={page ? Number(page) : 1} pageCount={pageCount} route="/blogs" />
        </div>
      )}
...
const getEvents = async ({ page = 1, limit }) => {
  try {
    const response = await fetch(`${API_URL}insight/blog/list?page=${page}&limit=${limit}`).then((res) => res.json());
    return response;
  } catch (error) {
