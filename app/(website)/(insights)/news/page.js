...
export const metadata = setMetaTitleAndDesc(
  "Insights & Blog | Millionones AI & Digital Marketing Kerala",
  "Stay updated with Millionones latest articles, insights, and trends on AI, digital marketing, branding, web design, and tech innovations from Kerala."
);
...
export default async function News(props) {
  ...
  const limit = 9;
  const { data = [], ...response } = await getNews({ page, limit });
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 py-10 mt-24 bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((blog, index) => (
            <BlogCard key={index} image={`${BASE_URL}${blog.image}`} title={blog.name} description={blog.desc} link={`/news/${blog.permalink}`} />
          ))}
        </div>
      </div>
      ...
      {pageCount > 1 && (
        <div className="flex justify-center bg-white">
          <InsightPagination page={page ? Number(page) : 1} pageCount={pageCount} route="/news" />
        </div>
      )}
...
const getNews = async ({ page = 1, limit }) => {
  try {
    const response = await fetch(`${API_URL}insight/news/list?page=${page}&limit=${limit}`).then((res) => res.json());
    return response;
  } catch (error) {
